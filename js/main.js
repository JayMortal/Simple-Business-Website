...
function showImageEditor(key, triggerEl) {
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:32px;max-width:480px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.4)">
      <h3 style="margin-bottom:4px;color:#0a1628;font-family:'Playfair Display',serif;font-size:1.2rem">${isEn ? 'Change Image' : '更换图片'}</h3>
      <p style="color:#888;font-size:.85rem;margin-bottom:20px">${isEn ? 'Language' : '当前语言'}：${lang === 'zh' ? '中文' : 'English'}</p>
      ...
      <div style="display:flex;gap:8px;margin-bottom:10px">
        <button onclick="document.getElementById('imgEditorPrev').src=document.getElementById('imgEditorUrl').value" style="flex:1;padding:9px;background:#f0f2f5;border:1px solid #ddd;border-radius:6px;cursor:pointer;font-size:.85rem">${isEn ? 'Preview URL' : '预览URL'}</button>
        <label style="flex:1;padding:9px;background:#f0f2f5;border:1px solid #ddd;border-radius:6px;cursor:pointer;font-size:.85rem;text-align:center">📤 ${isEn ? 'Upload' : '上传'}<input type="file" accept="image/*" style="display:none" onchange="(r=>{r.onload=e=>{document.getElementById('imgEditorPrev').src=e.target.result;document.getElementById('imgEditorUrl').value=e.target.result};r.readAsDataURL(this.files[0])})(new FileReader())"></label>
      </div>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
        <button onclick="this.closest('[style*=fixed]').remove()" style="padding:10px 24px;background:#f0f2f5;border:1px solid #ddd;border-radius:6px;cursor:pointer;font-family:inherit">${isEn ? 'Cancel' : '取消'}</button>
        <button onclick="applyImageEdit('${key}')" style="padding:10px 24px;background:#c9a84c;color:#0a1628;border:none;border-radius:6px;font-weight:700;cursor:pointer;font-family:inherit">${isEn ? 'Apply' : '应用'}</button>
      </div>
    </div>`;
...
function applyImageEdit(key) {
  ...
  showSiteToast(ui('图片已更新', 'Image updated'));
  window.syncToServer();
}

// ── Floating edit bar (bilingual)
function injectEditBar() {
  ...
  bar.innerHTML = `
    <span style="font-size:.85rem;opacity:.7">✏ ${isEn ? 'Edit Mode' : '编辑模式'}</span>
    <button onclick="openSidebarEditor()" style="padding:7px 18px;background:#c9a84c;color:#0a1628;border:none;border-radius:20px;font-weight:700;cursor:pointer;font-size:.85rem;font-family:inherit">${isEn ? 'Sidebar Editor' : '侧边栏编辑'}</button>
    <button onclick="saveFrontChanges()" style="padding:7px 18px;background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.3);border-radius:20px;cursor:pointer;font-size:.85rem;font-family:inherit">💾 ${isEn ? 'Save' : '保存'}</button>
    <a href="admin.html" style="padding:7px 18px;background:rgba(255,255,255,.1);color:#fff;border:1px solid rgba(255,255,255,.2);border-radius:20px;font-size:.85rem;text-decoration:none">⚙ ${isEn ? 'Admin' : '后台'}</a>
    <button onclick="exitAdminMode()" style="padding:7px 18px;background:rgba(255,80,80,.2);color:#ff9999;border:1px solid rgba(255,80,80,.3);border-radius:20px;cursor:pointer;font-size:.85rem;font-family:inherit">${isEn ? 'Exit' : '退出'}</button>`;
}
// Re-render bar if adminLang changes (e.g. user comes from admin with EN set)
window.addEventListener('storage', e => {
  if (e.key === 'adminLang' && isAdminMode()) injectEditBar();
});
...
function saveFrontChanges() {
  ...
  window.syncToServer(
    () => showSiteToast(ui('✅ 已保存并同步到服务器', '✅ Saved & synced to server')),
    () => showSiteToast(ui('✅ 已本地保存（服务器同步失败）', '✅ Saved locally (server sync failed)'))
  );
}
